...
const startMatchOrder=(playersRecord) => {
  ...
  (player1updated.allOps.includes(player2tocheckupdated.plyr))) {
    ...
    if(player1updated.allOps.length>2){
      const minPlays = Math.min(...playersRecord.map(obj => parseInt(obj.pys)));
      const newArray = playersRecord.filter(obj => parseInt(obj.pys) === minPlays).map(obj => obj.plyr);
